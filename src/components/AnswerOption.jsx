      ...
      {hasEmoji ? (
        <>
          <div className="flex items-center space-x-3 w-full pr-2">
            ...
            <span className={`text-[15px] font-semibold flex-1 leading-tight ${isLight ? 'text-gray-700' : 'text-white'}`}>
              {option.text}
            </span>
          </div>
          <svg className={`w-5 h-5 flex-shrink-0 ${isLight ? 'text-gray-400' : 'text-white/60'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M9 5l7 7-7 7"></path></svg>
        </>
      ) : (
        <span className={`text-[15px] font-bold w-full mx-auto block ${isLight ? 'text-gray-800' : 'text-white'}`}>
          {option.text}
        </span>
      )}
    </button>
